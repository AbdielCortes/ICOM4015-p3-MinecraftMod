...
public class TileEntityTreasureChest extends TileEntityLockableLoot implements ITickable {
	...
	public String getName() {
		return this.hasCustomName() ? this.customName : "container.treasure_chest";
	}
	...
	public String getGuiID() {
		return Reference.MOD_ID + ":treasure_chest";
	}
	...
	public void update() {
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0) {
            ...
            for (EntityPlayer entityplayer : this.world.getEntitiesWithinAABB(EntityPlayer.class, 
            		new AxisAlignedBB((double)((float)pos.getX() - 5.0F), 
            		(double)((float)pos.getY() - 5.0F), 
            		(double)((float)pos.getZ() - 5.0F), 
            		(double)((float)(pos.getX() + 1) + 5.0F), 
            		(double)((float)(pos.getY() + 1) + 5.0F), 
            		(double)((float)(pos.getZ() + 1) + 5.0F)))) {
                if (entityplayer.openContainer instanceof ContainerTreasureChest) {
                    if (((ContainerTreasureChest)entityplayer.openContainer).getChestInventory() == this) {
        ...
        if (this.numPlayersUsing == 0 && this.lidAngle > 0.0F || this.numPlayersUsing > 0 && this.lidAngle < 1.0F) {
            ...
            if (this.lidAngle < 0.5F && f2 >= 0.5F)
            {
                ...
                this.world.playSound((EntityPlayer)null, d3, (double)pos.getY() + 0.5D, d0, SoundEvents.BLOCK_ENDERCHEST_CLOSE, SoundCategory.BLOCKS, 0.5F, this.world.rand.nextFloat() * 0.1F + 0.9F);
            }
